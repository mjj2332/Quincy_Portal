...
//
//   node scripts/default-editors-backfill.mjs --manifest <dryrun.json> --out <apply.sql> [--run-at <ms>]
//
...
//   3. Dry-run first: `wrangler d1 execute ... --json --file default-editors-backfill-dryrun.sql
//      > dryrun.json`, and read it, before generating apply.sql from it.
 ...
 * @param {{ runAt?: number, newId?: () => string }} [options]
 */
export function buildBackfillSql(rows, options = {}) {
  ...
  if (!Number.isFinite(runAt) || runAt <= 0) throw new Error(`--run-at must be a positive integer ms timestamp: ${runAt}`);
  if (runAt > now) throw new Error(`--run-at must not be in the future: ${runAt}`);
  ...
  for (const projectId of projectOrder) {
    ...
    for (const userId of byProject.get(projectId)) {
      ...
      blocks.push(
        ...
        `SELECT '${sqlEscape(membershipId)}', '${sqlEscape(projectId)}', '${sqlEscape(userId)}', 'editor', ${runAt}\n` +
        `WHERE EXISTS (SELECT 1 FROM projects WHERE id='${sqlEscape(projectId)}' AND archived_at IS NULL)\n` +
        `  AND EXISTS (SELECT 1 FROM user WHERE id='${sqlEscape(userId)}' AND default_editor=1 AND active=1 AND role IN ('editor','external_editor','admin'))\n` +
    ...
    blocks.push(
      ...
      `  json_object('actor','operator_backfill','source','default_editor_backfill','userIds',(SELECT json_group_array(user_id) FROM (SELECT user_id FROM project_members WHERE id IN (${idList}) ORDER BY user_id))),\n` +
      `  ${runAt}\n` +
      `WHERE EXISTS (SELECT 1 FROM project_members WHERE id IN (${idList}))\n` +
      `  AND NOT EXISTS (SELECT 1 FROM audit_log WHERE action='project.default_editors.backfilled' AND target_type='project' AND target_id='${sqlEscape(projectId)}');`,
    );
  }
...
function parseArgs(argv) {
  const args = { runAt: undefined, manifest: undefined, out: undefined };
  for (let index = 0; index < argv.length; index += 1) {
    const arg = argv[index];
    if (arg === "--manifest") args.manifest = argv[++index];
    else if (arg === "--out") args.out = argv[++index];
    else if (arg === "--run-at") args.runAt = Number(argv[++index]);
...
async function main() {
  ...
  const sql = buildBackfillSql(rows, { runAt: args.runAt });
