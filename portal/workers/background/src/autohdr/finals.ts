  ...
  try {
    if (!current) {
      ...
      statements.push(
        ...
        env.DB.prepare("UPDATE projects SET stage_key = 'edited_review', updated_at = ? WHERE id = ? AND stage_key = 'editing_autohdr' AND archived_at IS NULL AND ? = 1 AND EXISTS (SELECT 1 FROM edited_source_claims WHERE collection_id = ? AND source_path_key = ? AND current_asset_id = ?)")
          .bind(now.getTime(), context.projectId, coverage ? 1 : 0, collectionId, sourcePathKey, assetId),
