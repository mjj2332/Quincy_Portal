...
- **Editor folder not moved** (`editor_folder_not_moved`, a reconcile skip reason): the Project's shoot date changed after its Editor tree was created and the tree is still under the old day folder. The mapping is untouched; the note names both dates and the root.
