 ...
 *
 * The navigation rail links Calendar to a parameterless URL and lets the Dashboard canonicalise it
 * from the remembered subview and last date it already reads. The alternative — the rail resolving
 * those preferences itself — was rejected because it would put the preference logic in two places.
 *
 * So this spelling is a deliberately transient *intent*, distinct from the parameterised facet URL
 * that `parseCalendarLocation` owns. It is legal only when `view` is the sole query field: anything
 * more is either the full facet (which must still parse as the facet) or an incomplete facet
 * (which must still be rejected, exactly as before).
 */
describe("the bare Calendar intent", () => {
  ...
  // The intent widens the grammar by exactly one spelling. Every partial facet stays rejected:
  // accepting `view=calendar` plus *some* of its parameters would silently discard the rest.
  it.each([
