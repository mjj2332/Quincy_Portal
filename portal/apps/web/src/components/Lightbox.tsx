...
export function Lightbox({ assets, rawAssets, initialAssetId, collectionKind, canReview, canRecommend, canAnnotate, onClose, onReview, onToast }: LightboxProps) {
  ...
  const move = (change: number) => {
    ...
    setIndex((current) => cycleLightboxIndex(current, change, assets.length));
  };
  ...
  return <div className={`viewer ${compareActive ? "viewer--compare" : ""}`} role="dialog" aria-modal="true" aria-label="Photo viewer">
    ...
    <div className="strip">{assets.map((item, itemIndex) => <FilmstripThumbnail key={item.id} asset={item} active={itemIndex === index} onSelect={() => { if (hasDraftMarkup && !window.confirm("Discard the current unsaved markup?")) return; setIndex(itemIndex); }} />)}</div>
  </div>;
}
