...
import { IconButton, ICON_BUTTON } from "./ui/icon-button";
...
export function Lightbox({ assets, rawAssets, initialAssetId, collectionKind, canReview, canRecommend, canAnnotate, onClose, onReview, onToast }: LightboxProps) {
  ...
  return <div className={cn("viewer", compareActive && "viewer--compare", "fixed inset-0 z-[80] bg-background grid grid-rows-[1fr_auto] grid-cols-[1fr_380px] max-[1081px]:grid-cols-[1fr]")} data-surface="inverse" role="dialog" aria-modal="true" aria-label="Photo viewer">
    ...
    <aside id="lightbox-review-panel" data-surface="default" className={`vpanel ${panelOpen ? "open" : ""}`}>
      ...
      <section className="vpanel__sec"><div className="eylab">Zoom</div><div className="row gap2"><IconButton type="button" onClick={() => setZoom((current) => zoomBy(current, -0.25))} disabled={zoom.scale <= 1} aria-label="Zoom out">−</IconButton><span className="muted" aria-live="polite">{Math.round(zoom.scale * 100)}%</span><IconButton type="button" onClick={() => setZoom((current) => zoomBy(current, 0.25))} disabled={zoom.scale >= 4} aria-label="Zoom in">+</IconButton><button className={buttonClasses("secondary")} type="button" onClick={resetZoom} disabled={zoom.scale === 1 && zoom.panX === 0 && zoom.panY === 0}>Reset</button></div><div className="muted" style={{ fontSize: 12, marginTop: 6 }}>{canAnnotate ? "Use the zoom buttons above; drag draws on the image." : "Scroll to zoom; drag a zoomed image to pan."}</div></section>
