...
export function Lightbox({ assets, rawAssets, initialAssetId, collectionKind, canReview, canRecommend, canAnnotate, onClose, onReview, onToast }: LightboxProps) {
  ...
  useEffect(() => {
    function keydown(event: KeyboardEvent) {
      ...
      if (event.key === "Escape" && isDrawingMode) { if (inEditableField) return; event.preventDefault(); exitDrawMode(); return; }
      ...
      if (isDrawingMode && !inEditableField && (event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "z") { event.preventDefault(); setStrokes((current) => current.slice(0, -1)); return; }
