...
const SHORTCUT_KBD =
  "inline-grid place-items-center min-w-[17px] px-[var(--space-1)] rounded-[var(--radius-xs)] " +
...
export function Lightbox({ assets, rawAssets, initialAssetId, collectionKind, canReview, canRecommend, canAnnotate, onClose, onReview, onToast }: LightboxProps) {
  ...
  return <div className={cn(
    ...
    {compareActive && rawCompareAsset ? <div className="viewer__stage row-start-1 row-end-2 relative grid place-items-center overflow-hidden min-w-0"><div className="absolute top-[16px] left-1/2 -translate-x-1/2 z-[4] text-center text-foreground max-[721px]:top-[calc(18px+env(safe-area-inset-top))] max-[721px]:max-w-[45vw]"><div className="[font:var(--type-h3)] [font-family:var(--font-display)] max-[721px]:hidden">RAW comparison</div><Eyebrow className="mt-[3px] text-on-inverse-muted max-[721px]:mt-0">{rawCompareAsset.originalFilename}</Eyebrow></div><div className="relative w-full h-full grid place-items-center overflow-hidden p-[28px] pb-[84px] max-[721px]:pt-[calc(28px+env(safe-area-inset-top))] max-[721px]:pb-[118px]"><div className={`canvasframe canvasframe--zoomable ${zoom.scale > 1 && !canAnnotate ? "is-zoomed" : ""}`} ref={rawFrameRef} data-testid="lightbox-canvas" style={zoomStyleFor(rawFrameRef)} onPointerDown={(event) => { startPan(event); startSwipe(event); }} onPointerMove={(event) => { movePan(event); moveSwipe(event); }} onPointerUp={(event) => { endSwipe(event); endPan(event); }} onPointerCancel={(event) => { swipeRef.current = null; endPan(event); }} onWheel={wheelZoom} onTouchStart={touchStartZoom} onTouchMove={touchMoveZoom} onTouchEnd={touchEndZoom}><img className="viewer__img max-w-full max-h-full object-contain select-none" draggable={false} onDragStart={(event) => event.preventDefault()} src={`/media/asset/${encodeURIComponent(rawCompareAsset.id)}/web`} alt={`RAW ${rawCompareAsset.originalFilename}`} /></div></div></div> : null}
    {editedStage}
    ...
    <aside id="lightbox-review-panel" data-surface="default" data-open={panelOpen ? "true" : undefined} className={cn("vpanel row-start-1 row-end-3 bg-background text-foreground border-l border-solid border-[length:var(--border-width-hair)] border-l-[var(--greige-500)] flex flex-col min-h-0 max-[1081px]:fixed max-[1081px]:right-0 max-[1081px]:top-0 max-[1081px]:bottom-0 max-[1081px]:w-[min(420px,90vw)] max-[1081px]:z-[6] max-[1081px]:shadow-[var(--shadow-lg)] max-[1081px]:translate-x-full max-[1081px]:transition-transform max-[1081px]:duration-[var(--dur-slow)] max-[1081px]:ease-[var(--ease-entrance)] max-[1081px]:[&.open]:translate-x-0", panelOpen && "open", compareActive && "col-start-3")}>
      {band === "phone" && !panelOpen ? <div className="vpanel__peek" onClick={(event) => openPanel(event.currentTarget)}>
        ...
        <div className="vpanel__head relative p-[var(--space-6)] pb-[var(--space-5)] border-b border-solid border-[length:var(--border-width-hair)] border-border"><div className="ey" style={{ marginBottom: 8 }}>{collectionKind === "edited" ? "Edited QA" : "RAW capture"} · {asset.width ?? "—"} × {asset.height ?? "—"}</div><div className="serif [font:var(--type-h3)] tracking-[var(--tracking-tight)] leading-[1.15]">{asset.originalFilename}</div>{band !== "desktop" && <button ref={collapseButtonRef} type="button" data-slot="icon-button" className={cn(ICON_BUTTON, "absolute top-[var(--space-4)] right-[var(--space-4)] min-h-[36px] min-w-[36px]")} onClick={closePanel} aria-label="Collapse review panel" aria-expanded={panelOpen} aria-controls="lightbox-review-panel">⌄</button>}</div><div className="overflow-auto flex-1 px-[var(--space-6)] py-[var(--space-5)] flex flex-col gap-[var(--space-5)]" data-testid="lightbox-review-scroll">
      <section className={PANEL_SECTION}><Eyebrow className="mb-[var(--space-3)]">Zoom</Eyebrow><div className="row gap2"><IconButton type="button" onClick={() => setZoom((current) => zoomBy(current, -0.25))} disabled={zoom.scale <= 1} aria-label="Zoom out">−</IconButton><span className="muted" aria-live="polite">{Math.round(zoom.scale * 100)}%</span><IconButton type="button" onClick={() => setZoom((current) => zoomBy(current, 0.25))} disabled={zoom.scale >= 4} aria-label="Zoom in">+</IconButton><button className={buttonClasses("secondary")} type="button" onClick={resetZoom} disabled={zoom.scale === 1 && zoom.panX === 0 && zoom.panY === 0}>Reset</button></div><div className="muted" style={{ fontSize: 12, marginTop: 6 }}>{canAnnotate ? "Use the zoom buttons above; drag draws on the image." : "Scroll to zoom; drag a zoomed image to pan."}</div></section>
      {band !== "phone" && collectionKind === "edited" && rawCompareAsset && <section className={PANEL_SECTION}><Eyebrow className="mb-[var(--space-3)]">RAW ↔ Edited</Eyebrow><button className={cn(buttonClasses(compareActive ? "primary" : "secondary", { className: "w-full" }))} type="button" aria-pressed={compareActive} onClick={() => { resetZoom(); setShowRawCompare((current) => !current); }}>{compareActive ? "Hide RAW comparison" : "Compare with RAW"}</button></section>}
      {canReview && <section className={PANEL_SECTION}><Eyebrow className="mb-[var(--space-3)]">Decision</Eyebrow><div className="grid grid-cols-2 gap-[var(--space-2)]"><button className={cn("on-approve", buttonClasses(asset.review?.decision === "approved" ? "primary" : "secondary", { className: "w-full" }))} type="button" aria-pressed={asset.review?.decision === "approved"} onClick={() => void onReview(asset.id, { decision: asset.review?.decision === "approved" ? null : "approved" })}>✓ Approve</button><button className={cn("on-flag", buttonClasses(asset.review?.decision === "flagged" ? "danger" : "secondary", { className: "w-full" }))} type="button" aria-pressed={asset.review?.decision === "flagged"} onClick={() => void onReview(asset.id, { decision: asset.review?.decision === "flagged" ? null : "flagged" })}>⚑ Flag</button></div></section>}
