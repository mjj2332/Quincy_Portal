...
const SHORTCUT_KBD =
  ...
  "[font:var(--weight-regular)_var(--text-2xs)/1.4_var(--font-mono)]";
...
export function Lightbox({ assets, rawAssets, initialAssetId, collectionKind, canReview, canRecommend, canAnnotate, onClose, onReview, onToast }: LightboxProps) {
  ...
  return <div className={cn(
    ...
    <aside id="lightbox-review-panel" data-surface="default" data-open={panelOpen ? "true" : undefined} className={cn("vpanel row-start-1 row-end-3 bg-background text-foreground border-l border-solid border-[length:var(--border-width-hair)] border-l-[var(--greige-500)] flex flex-col min-h-0 max-[1081px]:fixed max-[1081px]:right-0 max-[1081px]:top-0 max-[1081px]:bottom-0 max-[1081px]:w-[min(420px,90vw)] max-[1081px]:z-[6] max-[1081px]:shadow-[var(--shadow-lg)] max-[1081px]:translate-x-full max-[1081px]:transition-transform max-[1081px]:duration-[var(--dur-slow)] max-[1081px]:ease-[var(--ease-entrance)] max-[1081px]:[&.open]:translate-x-0", panelOpen && "open", compareActive && "col-start-3")}>
      ...
      <section className={PANEL_SECTION}><div className={cn("eylab", SECTION_LABEL, "flex justify-between items-center")}><span>Markup & annotations</span><span style={{ display: "flex", gap: 6 }}><button className="chip" type="button" onClick={() => setMarkupVisible((current) => !current)}>{markupVisible ? "Hide markup" : "Show markup"}</button></span></div>{canAnnotate && <><Textarea className="annotation-note mt-[var(--space-3)]" placeholder="Optional note for this markup…" value={annotationNote} disabled={editingDrawingId !== null || editingAnnotationId !== null} onChange={(event) => setAnnotationNote(event.target.value)} /><button className={cn("dbtn", buttonClasses("primary", { className: "w-full mt-[var(--space-2)]" }))} type="button" disabled={isSaving || editingDrawingId !== null || (!strokes.length && !annotationNote.trim())} onClick={() => void saveAnnotation()}>Save annotation</button></>}<div className="thread flex flex-col gap-[var(--space-4)]" style={{ marginTop: 14 }}>{annotations.length === 0 ? <div className="muted" style={{ fontSize: 14 }}>No annotations yet.</div> : annotations.map((annotation) => <div className={cn("cmt flex gap-[var(--space-3)] rounded-[var(--radius-sm)] transition-[background-color] duration-[1600ms] ease-[var(--ease-standard)]", highlightedAnnotationId === annotation.id && "cmt--highlighted bg-surface-sunken")} data-highlighted={highlightedAnnotationId === annotation.id ? "true" : undefined} key={annotation.id} ref={(el) => { if (el) annotationRefs.current.set(annotation.id, el); else annotationRefs.current.delete(annotation.id); }}><div aria-hidden="true" className={cn("cmt__pin flex-none w-[22px] h-[22px] rounded-full bg-primary text-primary-foreground grid place-items-center [font:var(--weight-regular)_var(--text-2xs)/1.4_var(--font-mono)] mt-[2px]")}>✎</div><div className="cmt__b flex-1 min-w-0"><div className="cmt__who [font:var(--type-label)]">{annotation.author.name}<span className={cn(META_TEXT, "ml-[var(--space-2)]")}>{annotation.author.role}</span></div>{editingAnnotationId === annotation.id ? <><Textarea className="annotation-note mt-[var(--space-3)]" aria-label="Edit annotation note" value={editingAnnotationNote} disabled={isSaving} onChange={(event) => setEditingAnnotationNote(event.target.value)} /><div className="spread" style={{ marginTop: 6 }}><button className={cn("comment-reply", buttonClasses("text", { className: "underline min-w-[44px] max-[721px]:min-h-[44px] px-[var(--space-1)]" }))} data-testid="lightbox-annotation-action" type="button" disabled={isSaving} onClick={cancelInlineEdit}>Cancel</button><button className={cn("comment-reply", buttonClasses("text", { className: "underline min-w-[44px] max-[721px]:min-h-[44px] px-[var(--space-1)]" }))} data-testid="lightbox-annotation-action" type="button" disabled={isSaving} onClick={() => void saveAnnotationEdit()}>Save</button></div></> : <>{annotation.noteText && <div className="cmt__txt text-[length:var(--text-sm)] leading-[var(--leading-normal)] mt-[3px] text-foreground-secondary">{annotation.noteText}</div>}<div className="ey muted" style={{ marginTop: 6 }}>{time(annotation.createdAt)}{annotation.editedAt && " · (edited)"}{annotation.authorId === currentUserId && <> <span aria-hidden="true">·</span> <button className={cn("comment-reply", buttonClasses("text", { className: "underline min-w-[44px] max-[721px]:min-h-[44px] px-[var(--space-1)]" }))} data-testid="lightbox-annotation-action" type="button" disabled={hasDraftMarkup || isSaving} onClick={() => { setEditingAnnotationId(annotation.id); setEditingAnnotationNote(annotation.noteText ?? ""); }}>Edit note</button> <span aria-hidden="true">·</span> <button className={cn("comment-reply", buttonClasses("text", { className: "underline min-w-[44px] max-[721px]:min-h-[44px] px-[var(--space-1)]" }))} data-testid="lightbox-annotation-action" type="button" disabled={hasDraftMarkup || isSaving || !strokesReady(annotation)} title={!strokesReady(annotation) ? "Loading markup…" : undefined} onClick={() => startDrawingEdit(annotation)}>{annotation.strokeR2Key ? "Edit drawing" : "Add drawing"}</button> <span aria-hidden="true">·</span> <button className={cn("comment-reply", buttonClasses("text", { className: "underline min-w-[44px] max-[721px]:min-h-[44px] px-[var(--space-1)]" }))} data-testid="lightbox-annotation-action" type="button" disabled={hasDraftMarkup || isSaving} onClick={() => void deleteAnnotation(annotation)}>Delete</button></>}</div></>}</div></div>)}</div></section>
    </div></> : null}</aside>
    <div className={cn(
