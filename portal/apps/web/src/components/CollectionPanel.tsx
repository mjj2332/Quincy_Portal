...
const LINK_FORM_BASE = "grid grid-cols-1 gap-[var(--space-3)] items-end min-w-0";
...
function LinkTile({ link, sortable = false, busy = false, onRemove, onEdit, editingLinkId, editDraft, editError, savingEdit, onEditDraftChange, onSaveEdit, onCancelEdit }: {
  ...
  const content = <><span className="[font:var(--weight-regular)_var(--text-lg)/var(--leading-snug)_var(--font-display)] tracking-[var(--tracking-tight)] text-foreground [overflow-wrap:anywhere]" data-testid="collection-link-name">{title}</span><span className="chip" data-testid="collection-link-source">{hostLabel(link.url)}</span></>;
  ...
  return <div ref={sortable ? setNodeRef : undefined} style={sortable ? { transform: CSS.Transform.toString(transform), transition } : undefined} data-testid="collection-link" className={cn(
    "flex flex-col justify-between min-h-[var(--space-10)] p-[var(--space-4)]",
  ...
  )}>{isEditing && editDraft && onEditDraftChange && onSaveEdit && onCancelEdit ? <form className={cn(LINK_FORM_BASE, "p-0 border-0 bg-transparent")} data-testid="collection-link-editor" onSubmit={onSaveEdit}><label className={LINK_FORM_LABEL}><span>Label <em className="not-italic text-foreground-secondary">optional</em></span><input className={RAIL_FIELD} aria-describedby={editError ? errorId : undefined} placeholder="Final walkthrough" value={editDraft.label} onChange={(event) => onEditDraftChange({ ...editDraft, label: event.target.value })} /></label><label className={LINK_FORM_LABEL}><span>Link URL</span><input className={RAIL_FIELD} required type="url" aria-describedby={editError ? errorId : undefined} placeholder="https://vimeo.com/…" value={editDraft.url} onChange={(event) => onEditDraftChange({ ...editDraft, url: event.target.value })} /></label>{editError && <p id={errorId} className={cn(LINK_ERROR_TEXT)} role="alert">{editError}</p>}<div className="flex flex-wrap gap-[var(--space-2)]"><button className={buttonClasses("primary", { className: "min-h-[44px]" })} disabled={savingEdit}>{savingEdit ? "Saving…" : "Save"}</button><button className={buttonClasses("secondary", { className: "min-h-[44px]" })} type="button" disabled={savingEdit} onClick={onCancelEdit}>Cancel</button></div></form> : <>{safeUrl ? <a href={safeUrl.href} target="_blank" rel="noopener noreferrer" className={LINK_BODY}>{content}</a> : <div className={cn(LINK_BODY)} data-testid="collection-link-plain">{content}</div>}<div className="flex items-center justify-between gap-[var(--space-2)] mt-[var(--space-4)]" data-testid="collection-link-meta">{sortable && <button ref={setActivatorNodeRef} type="button" className={LINK_GRIP} aria-label={`Reorder ${title}`} {...dragProps}><GripVertical aria-hidden="true" className="size-[var(--space-4)] stroke-[1.5]" /></button>}<span className={cn(
