...
function LinkTile({ link, sortable = false, busy = false, onRemove, onEdit, editingLinkId, editDraft, editError, savingEdit, onEditDraftChange, onSaveEdit, onCancelEdit }: {
  ...
  const content = <><span className="collection-link__name [font:var(--weight-regular)_var(--text-lg)/var(--leading-snug)_var(--font-display)] tracking-[var(--tracking-tight)] text-foreground [overflow-wrap:anywhere]">{title}</span><span className="chip">{hostLabel(link.url)}</span></>;
  const isEditing = editingLinkId === link.id;
  const errorId = `collection-link-edit-error-${link.id}`;
  const dragProps = { ...attributes, ...listeners, ...(busy ? { "aria-disabled": true } : {}) };
  return <div ref={sortable ? setNodeRef : undefined} style={sortable ? { transform: CSS.Transform.toString(transform), transition } : undefined} className={cn(
