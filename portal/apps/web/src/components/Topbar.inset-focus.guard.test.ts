/**
 * Inset-focus guard — pins the `!` on `Topbar.tsx`'s `HIGHLIGHT_STATE` focus utilities as SOURCE
 * TEXT, not as a DOM query.
 *
 * `HIGHLIGHT_STATE` is worn by every menu option, the dismiss control, the mobile items and the
 * "Mark all read" head button. Those sit flush inside a bordered, `overflow-auto` panel, so their
 * focus ring must render INWARD or it clips against the panel edge.
 *
 ...
 * `Topbar.dom.test.tsx` can see an offset flip. Same mechanism and same fix as
...
const topbarSourcePath = join(componentsDir, "Topbar.tsx");

function topbarSource(): string {
  return readFileSync(topbarSourcePath, "utf8");
}
 ...
 * would pass on a stray `!outline` anywhere in `Topbar.tsx`.
 *
...
describe("Topbar HIGHLIGHT_STATE inset focus ring", () => {
  it("declares an inward offset", () => {
    const initialiser = highlightStateInitialiser(topbarSource());
