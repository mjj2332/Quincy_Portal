...
//
// 5. cva base: nova's focus ring (`focus-visible:border-ring focus-visible:ring-3
//    focus-visible:ring-ring/50`) is REMOVED, and with it `variants.variant.destructive`'s
//    `focus-visible:border-destructive/40 focus-visible:ring-destructive/20
//    dark:focus-visible:ring-destructive/40`, which existed only to recolour it.
//
//    This follows directly from 4. The global `:focus-visible` outline paints on EVERY focusable
//    element, so nova's ring is not the focus indicator here — it is a SECOND one. The two live in
//    different CSS property groups (`outline` vs `border-color`/`box-shadow`), so tailwind-merge
//    cannot collapse them and both survive every merge: a doubled ring on every control that wears
//    `buttonVariants`. The legacy `ui/button.tsx` never showed this, because its own ring was an
//    `outline` and therefore merged with the global one down to a single winner.
//
//    `aria-invalid:ring-3` is deliberately KEPT. It is an error affordance, not a focus indicator;
//    it paints on an invalid control whether or not that control is focused.
//
