...
// Deliberately not SCREAMING_CASE, unlike the vendor original: `design-system-guards.test.ts`'s
// font-size collision guard scans every non-test source file for constants that might carry a
// `[font:…]` shorthand with `const\s+([A-Z][A-Z0-9_]*)\s*(?::[^=]+)?=\s*((?:[^;]|\n)*?);`. Its
// `(?:[^;]|\n)*?` is ambiguous on every newline it crosses — `[^;]` already matches `\n` — and
// once the identifier half matches a SCREAMING_CASE const it lazily expands hunting for this
// file's next literal `;`. The vendor file below is otherwise semicolon-free, so that search ran
// off for hundreds of statement-free newlines: textbook catastrophic backtracking, hanging the
// guard suite indefinitely (immune to `--testTimeout` — the hang is synchronous regex work, not
  ...
  const columnIds = useMemo(() => {
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      for (const key of keys) {
        // `key` always comes from `Object.keys(columns)` (here and at every other bare index
        // below), so the value is always present — `noUncheckedIndexedAccess` cannot see that.
