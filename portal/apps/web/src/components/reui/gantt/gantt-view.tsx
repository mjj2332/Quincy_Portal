 ...
 * is fixed with the smallest possible edit, a non-null assertion, marked inline at its site with
 * a one-line comment naming the invariant that makes it safe — search this file for "Quincy
...
function GanttView({
  ...
  // Quincy edit (#219 stage 1): non-null assertion. `.every` only invokes its callback when
  // `units` is non-empty, so `units[0]` is defined whenever this line runs; Quincy's stricter
  ...
  const resolveHintStop = useMemo(() => {
    return (
      ...
      for (let i = 0; i < unitFractions.length; i++) {
        // Quincy edit (#219 stage 1): non-null assertion. `i < unitFractions.length` guarantees
        // this index is defined; Quincy's stricter `noUncheckedIndexedAccess` (unset in the
        // vendor's own tsconfig) does not know that.
  ...
  const groupBoundaries = useMemo(() => {
    ...
    for (let i = 0; i < groups.length - 1; i++) {
      // Quincy edit (#219 stage 1): non-null assertion. `i < groups.length - 1` guarantees this
      // index is defined; Quincy's stricter `noUncheckedIndexedAccess` (unset in the vendor's
      // own tsconfig) does not know that.
      acc += groups[i]!.span
  ...
  const baseRowBars = useMemo(() => {
    ...
    for (const row of rows) {
      ...
      map.set(row.resource.id, {
        ...
        extent:
          // Quincy edit (#219 stage 1): non-null assertions on `segments[0]` (x2 below). The
          // `segments.length > 0` guard makes index 0 always defined in this branch; Quincy's
          // stricter `noUncheckedIndexedAccess` (unset in the vendor's own tsconfig) does not
          // know that.
          segments.length > 0
            ? {
                from,
  ...
  const beginRowReorder = useCallback(
    (e: React.PointerEvent, dragRow: TimelineRow) => {
      ...
      const propose = (boundary: number): TimelineReorderState => {
        ...
        let index = 0
        // Quincy edit (#219 stage 1): non-null assertions on `rows[i]` (x2 below). `boundary` is
        // an index into `rows` (see its callers), so `i < boundary` guarantees this index is
        // defined; Quincy's stricter `noUncheckedIndexedAccess` (unset in the vendor's own
        // tsconfig) does not know that.
        ...
        // Quincy edit (#219 stage 1): non-null assertion. `boundary < rects.length` guarantees
        // this index is defined; Quincy's stricter `noUncheckedIndexedAccess` (unset in the
        // vendor's own tsconfig) does not know that.
        const top =
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        // Quincy edit (#219 stage 1): non-null assertions on `rects[i]` (x2 below). `i <
        // rects.length` guarantees this index is defined; Quincy's stricter
        // `noUncheckedIndexedAccess` (unset in the vendor's own tsconfig) does not know that.
...
function sameChips(a: OffscreenChip[], b: OffscreenChip[]): boolean {
  // Quincy edit (#219 stage 1): non-null assertions on `b[i]` (x4 below). The `a.length ===
  // b.length` guard makes every index `.every` visits on `a` valid on `b` too; Quincy's
  // stricter `noUncheckedIndexedAccess` (unset in the vendor's own tsconfig) does not know that.
  return (
    a.length === b.length &&
    a.every(
      (chip, i) =>
