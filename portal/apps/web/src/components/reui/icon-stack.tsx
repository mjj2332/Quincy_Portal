...
/**
 * IconStack — ReUI's `@reui/icon-stack`, fetched via `npx shadcn@latest add @reui/icon-stack`
 * into a sandbox (`tmp/ReUI-Test-1`) for #219 (stage 1 of the Gantt vendor, PR A). Two edits, the
 * first mechanical:
 *
 ...
 * 2. **Added `import * as React from "react"`.** The registry source uses `React.ComponentProps`
 *    and `React.CSSProperties` without importing `React` at all — same gap as
 *    `reui/button-group.tsx`'s header describes, and the same fix. No `"use client"` directive
 *    was present to drop.
 */
