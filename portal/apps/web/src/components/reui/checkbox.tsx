// As installed by `shadcn add checkbox` under the base-nova style, with one correction: the
// registry item ships `import { cn } from "cn"` and declares "cn" as an npm dependency. `cn` is a
// real published package, so the CLI installed it — silently shadowing this repo's own
// `@/lib/utils` helper, which is twMerge-backed and is what every other component here uses.
// Reverted; do not let a re-install reintroduce it.
  ...
  return (
    <CheckboxPrimitive.Root
      ...
      className={cn(
        ...
        //
        // `disabled:*` -> `data-disabled:*`. Base UI's root is a <span>, so the :disabled
        // pseudo-class NEVER matches it and both vendor declarations were dead. That is not
