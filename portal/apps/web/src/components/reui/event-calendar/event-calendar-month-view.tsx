 ...
 *
 * THIS FILE: the month view — an ARIA grid of week rows and day cells, with continuous multi-day bars in a lane overlay, the "+N more" overflow popover, week numbers, and the per-cell add affordance.
 *
 *   - Its overflow popover is styled through `**:data-[slot=scroll-area-viewport]`, which only
 *     resolves because Quincy's `components/reui/scroll-area.tsx` authors that exact slot name.
 *     That is a real coupling between this file and a primitive outside this directory: a
 *     scroll-area version bump that renames the slot breaks this silently, with no type error.
 *
 * Quincy edits since vendoring:
 *
 * 1. 2026-09-20, #219 PR B, stage 1 — type-only narrowing for this repo's
 *    `noUncheckedIndexedAccess: true`, which the registry does not compile under. 7 sites:
 *    `week[0]` hoisted into a `firstDay` local proven non-empty by the row's own contract,
...
function EventCalendarMonthWeek({
  ...
  const settings = useEventCalendarSettings()
  const viewConfig = useEventCalendarViewConfig()
  // A month/week row always renders at least one day.
