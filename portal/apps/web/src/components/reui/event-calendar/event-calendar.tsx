 ...
 *
 * THIS FILE: the root provider and the engine: `createEventCalendarStore` / `useEventCalendarState`, the settings and view-config resolution, the imperative `EventCalendarApi`, and the deliberately non-public `EventCalendarInternals` the view and gesture files read.
 *
 *   - `applyProposedUpdate` maps over EXISTING events by id — it can only ever update one, never
 *     create. Anything that needs to ADD an event (an external drop, for instance) goes through
 *     `api.addEvent`, which is an unchecked append, not an acceptance funnel. Do not mistake one
 *     for the other.
 *   - `EventCalendarInternals` is marked "not public API" upstream. #219 PR B relies on it
