 ...
 *    `noUncheckedIndexedAccess: true`, which the registry does not compile under. 11 sites:
 *    the lane-packing `lanes[lane]` writes hoisted into a local, the memoized bars'
 *    `isEqual` `b.bars[i]` asserted non-null since the lengths are already compared,
  ...
  >(
    ...
    },
    {
      ...
      isEqual: (a, b) =>
        ...
        a.bars.every((s, i) => {
          // a.bars.length === b.bars.length was already checked above, so
          // `i` always indexes `b.bars` too.
          const o = b.bars[i]!
...
function EventCalendarAllDayBars({
  ...
  const settings = useEventCalendarSettings()
  const viewConfig = useEventCalendarViewConfig()
  // The time grid always renders at least one day.
  ...
  >(
    (state) => {
      ...
      return {
        ...
        // colStart/colEnd were set from `i` while iterating `days`, so both
        // always index it.
                ...
                {dragGhost.kind !== "move" && (
                  <EventCalendarEvent
                    ...
                    segment={{
                      ...
                      day: days[dragGhost.colStart] ?? firstDay,
