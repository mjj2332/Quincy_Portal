 ...
 *
 * THIS FILE: the RFC 5545 subset expander — structured rules or a raw `RRULE:` line, with `exDates` / `rDates`, stepping on zoned wall-clock so a series survives a DST transition.
 *
 * Quincy edits since vendoring:
 *
 * 1. 2026-09-20, #219 PR B, stage 1 — type-only narrowing for this repo's
 *    `noUncheckedIndexedAccess: true`, which the registry does not compile under. 2 sites:
 *    the `y`/`m`/`d` regex capture groups asserted non-null since they are mandatory
 *    groups the `UNTIL` regex already requires, and `yearlyMonths[0]`/
 *    `yearlyMonths[...length - 1]` asserted
 *    non-null since that array is always non-empty. No runtime behaviour changed.
 */
import type {
  CalendarEvent,
...
function parseRRuleDate(value: string, timeZone?: string): Date {
  ...
  const date = z
    ...
    : timeZone
      ? // y, m, d are mandatory (non-optional) regex capture groups, so once
        // `match` succeeded they are always defined.
        new Date(
...
function expandRecurrence<TData>(
  ...
  const periodEdge = (period: number, edge: "first" | "last"): TZDate => {
    ...
    const year = addYears(zonedStart, period * interval).getFullYear()
    const month =
      // yearlyMonths is never empty: it's either a deduped non-empty filter
      // result or a single-element default.
      (edge === "first"
        ? yearlyMonths[0]!
