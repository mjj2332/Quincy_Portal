 ...
 *
 * THIS FILE: the navigation bar — Today, prev/next, the live-region title, the view switcher, the optional go-to-date popover, and the free toolbar slot.
 *
 *   - The date picker passes a `date-fns` `Locale` (from `event-calendar-i18n.tsx`) into Quincy's
 *     adapted `calendar.tsx`, which types `locale` through react-day-picker's own `Locale`. The
 *     two are structurally compatible today; a react-day-picker bump is the thing that would
 *     break it.
 *
