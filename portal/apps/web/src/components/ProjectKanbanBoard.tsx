...
export function KanbanCard({
  ...
  return <div ref={setCardRef} style={cardStyle} className="relative bg-card border border-border transition-[background-color,border-color] duration-[var(--dur-fast)] hover:bg-[var(--paper-100)] hover:border-[var(--greige-300)] data-[dragging=true]:opacity-40" data-testid="kanban-card-wrap" data-dragging={isDragging ? "true" : undefined}>
    <InternalLink className="kcard w-full block p-0 text-inherit text-left [font:inherit] no-underline bg-none border-0 cursor-pointer" data-testid="kanban-card" to={projectHref ?? `/projects/${encodeURIComponent(project.id)}`}>
      <div className="aspect-[16/9] overflow-hidden bg-[var(--ink-800)]"><CoverMedia project={project} className="size-full object-cover" retryToken={coverRetry} onFailedChange={setCoverFailed} /></div>
      <div className="p-[var(--space-3)]">
        <div className="serif text-base tracking-tight leading-snug [text-wrap:pretty]" data-testid="kanban-card-address">{project.street}</div>
        <div className="text-xs text-foreground-secondary mt-[var(--space-1)]">{location(project)}</div>
        <div className="text-xs text-foreground-secondary mt-[var(--space-1)]">{project.agencyName || "Agency pending"}</div>
        <div className="flex items-center flex-wrap gap-[var(--space-1)_var(--space-3)] mt-[var(--space-3)]" data-testid="kanban-card-footer">
          {projectDeadlineLabel && <time className={overdue ? "project-deadline__overdue" : ""} data-testid="kanban-card-deadline" dateTime={new Date(project.deadlineAt!).toISOString()}>{overdue ? "Overdue" : "Due"} {projectDeadlineLabel} Sydney</time>}
    ...
    {canPrioritize && <div className="kcard-controls flex items-center gap-[var(--space-1)] px-[var(--space-3)] pb-[var(--space-3)] max-[641px]:flex-wrap pointer-coarse:flex-wrap" aria-label={`Order controls for ${project.street}`}>
      ...
      {canReorder && <>
        ...
        <button type="button" className="w-7 h-[26px] max-[641px]:size-11 max-[641px]:min-w-11 pointer-coarse:size-11 pointer-coarse:min-w-11 p-0 border border-[length:var(--border-width-hair)] border-border rounded-[var(--radius-sm)] bg-[var(--bg-surface)] text-foreground-secondary [font:var(--type-label)] text-sm leading-none cursor-pointer transition-[background-color,color] duration-[var(--dur-fast)] ease-[var(--ease-standard)] focus-visible:!outline focus-visible:!outline-[length:var(--border-width-bold)] focus-visible:!outline-[var(--focus-ring)] focus-visible:!outline-offset-2 hover:not-disabled:bg-[var(--bg-raised)] hover:not-disabled:text-foreground active:not-disabled:translate-y-px disabled:text-foreground-secondary disabled:bg-surface-sunken disabled:cursor-not-allowed" data-testid="kanban-card-control" data-focus-key={`arrow-down:${project.id}`} aria-label="Move project down" disabled={movementDisabled} onClick={() => onBoardPosition?.(project, "down")}>↓</button>
      </>}
    </div>}
    {canMove && <div className="px-[var(--space-3)] pb-[var(--space-3)]">
    ...
    {coverFailed && <button className={buttonClasses("secondary", { className: "mt-[var(--space-2)] mx-[var(--space-3)] mb-[var(--space-3)]" })} type="button" onClick={() => { setCoverFailed(false); setCoverRetry((current) => current + 1); }}>Retry cover image</button>}
  </div>;
}
...
export function KanbanCardPreview({ project }: { project: ProjectSummary }) {
  const overdue = isDeadlineOverdue(project.deadlineAt);
  const projectDeadlineLabel = deadlineLabel(project);
  return <div className="w-[min(320px,calc(100vw-var(--space-5)))] max-w-[calc(100vw-var(--space-5))] box-border shadow-[0_18px_36px_color-mix(in_srgb,var(--ink-900)_22%,transparent)] pointer-events-none relative bg-card border border-border transition-[background-color,border-color] duration-[var(--dur-fast)]" data-testid="kanban-card-wrap" aria-hidden="true">
    <div className="aspect-[16/9] overflow-hidden bg-[var(--ink-800)] pointer-events-none"><CoverMedia project={project} className="size-full object-cover" /></div>
    <div className="p-[var(--space-3)]">
      <div className="serif text-base tracking-tight leading-snug [text-wrap:pretty]" data-testid="kanban-card-address">{project.street}</div>
      <div className="text-xs text-foreground-secondary mt-[var(--space-1)]">{location(project)}</div>
      <div className="text-xs text-foreground-secondary mt-[var(--space-1)]">{project.agencyName || "Agency pending"}</div>
...
function KanbanColumn({
  ...
  return <section className="group bg-[var(--paper-050)] flex flex-col min-w-0 transition-[background-color] duration-[var(--dur-fast)] data-[over=true]:bg-[var(--paper-100)]" data-testid="kanban-column" data-over={stageIsOver ? "true" : undefined}>
    <div className="kcol__head flex items-center gap-[var(--space-3)] p-[var(--space-4)] border-b border-b-border bg-[var(--bg-canvas)] group-data-[over=true]:bg-[var(--paper-100)] [&_.ey]:!leading-[1.25] focus-visible:!outline focus-visible:!outline-[length:var(--border-width-bold)] focus-visible:!outline-[var(--focus-ring)] focus-visible:!outline-offset-[-2px]" data-focus-key={`stage-heading:${semanticKey}`} tabIndex={-1}><span className="flex-none [font:var(--type-eyebrow)] uppercase tracking-[var(--tracking-wide)] tabular-nums text-foreground-secondary" aria-hidden="true">{String(stageIndex + 1).padStart(2, "0")}</span><StatusBadge stageKey={stage.key} /><span className="cnt flex-none tabular-nums text-sm text-foreground-secondary">{displayedProjects.length}</span></div>
    <SortableContext items={displayedIds} strategy={verticalListSortingStrategy}>
      ...
      <div ref={setColumnBodyRef} className="flex flex-col gap-[var(--space-3)] p-[var(--space-3)] min-h-[120px] flex-1" data-droppable-id={`column:${stage.key}`}>
        {displayedProjects.length === 0 && <div className="py-[var(--space-5)] [font-family:var(--font-display)] text-lg text-center text-foreground-secondary">—</div>}
        {displayedProjects.map((project) => {
          ...
          return <div key={project.id}>
            {isDropBefore && <div className="min-h-[3px] rounded-[2px] bg-[var(--signal-positive)] shadow-[0_0_0_1px_color-mix(in_srgb,var(--signal-positive)_20%,transparent)]" data-testid="kanban-drop-indicator" aria-hidden="true" />}
            <SortableKanbanCard
              {...{
        ...
        {indicatorAfter && <div className="min-h-[3px] rounded-[2px] bg-[var(--signal-positive)] shadow-[0_0_0_1px_color-mix(in_srgb,var(--signal-positive)_20%,transparent)]" data-testid="kanban-drop-indicator" aria-hidden="true" />}
      </div>
    </SortableContext>
  </section>;
}
