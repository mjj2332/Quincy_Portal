...
export function ProjectCollaborationPanel({ projectId, openSignal, onOpenSignalConsumed, mode = "overlay", onAccessFailure }: ProjectCollaborationPanelProps) {
  ...
  const renderDiscussion = ({ content, project, scrollRootRef }: Parameters<NonNullable<React.ComponentProps<typeof ProjectDiscussionThread>["children"]>>[0]) => {
    ...
    if (!overlay) return <section className="project-collaboration project-collaboration--standalone" data-testid="project-collaboration-panel" data-mode="standalone" aria-label="Project collaboration">{headerMarkup}{tabs}{panels}</section>;
    return <section className="project-collaboration__wrap" data-testid="project-collaboration-wrap"><button ref={triggerRef} type="button" className="project-collaboration__toggle" data-testid="project-collaboration-toggle" aria-label={toggleLabel} aria-expanded={open} aria-controls={`project-collaboration-${projectId}`} onClick={() => setOverlayOpen((value) => !value)}><span aria-hidden="true">Collaboration</span>{unreadCount > 0 && <span data-testid="project-collaboration-unread" className={UNREAD_BADGE} aria-hidden="true">{unreadLabel}</span>}</button>{open && <aside ref={panelRootRef} id={`project-collaboration-${projectId}`} className="project-collaboration project-collaboration--overlay" data-testid="project-collaboration-panel" data-mode="overlay" aria-label="Project collaboration">{headerMarkup}{tabs}<div ref={scrollRootRef} data-testid="project-collaboration-scroll" className="grid content-start gap-[var(--space-4)] min-h-0 flex-1 overflow-auto p-[var(--space-5)]">{panels}</div></aside>}</section>;
  };
