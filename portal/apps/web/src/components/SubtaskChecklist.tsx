...
// `reui/input`'s FIELD_BOX carries the outward ring `buttonClasses`'s `insetFocus` neutralises on buttons
// (`focus-visible:ring-3 focus-visible:ring-ring/50`), and twMerge cannot collapse it against
// RING_IN's `!outline` — different property groups. Only the ring is zeroed here, NOT
// `focus-visible:border-ring` as the button-side fix does: a border recolour sits inside the box and
// is never clipped by AnchoredPopover's `overflow:auto` panel, whereas blanking the border leaves
// the field borderless while focused. Not exported, per §4.3.
...
function ActionsControl({ owner, title, open, setOpen, busy, onDelete }: { owner: string; title: string; open: boolean; setOpen: (open: boolean) => void; busy: boolean; onDelete: () => void }) {
  ...
  return <>
    ...
    {floating.mounted && <AnchoredPopover context={floating.context} floatingStyles={floating.floatingStyles} initialFocus={0} onKeyDown={floating.onKeyDown} status={floating.status}><div id={id} className={POPOVER_CONTENT} role="group" aria-label={`Actions for ${title}`}><button type="button" className={buttonClasses("danger", { className: RING_IN, insetFocus: true })} disabled={busy} onClick={() => { void (async () => { if (!await confirm({ title: "Delete subtask?", message: "Delete this subtask?", confirmLabel: "Delete", danger: true })) return; close(); onDelete(); })(); }}>Delete</button></div></AnchoredPopover>}
  </>;
}
