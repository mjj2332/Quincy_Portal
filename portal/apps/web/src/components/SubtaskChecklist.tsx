...
export function SubtaskChecklist({ projectId, onAccessFailure }: { projectId: string; onAccessFailure?: (error: unknown) => void }) {
  ...
  async function invalidateAfterMutation() { if (queryClient) await invalidateProjectSurfaces(queryClient, { projectId, resources: [{ kind: "subtasks" }, { kind: "activity" }], dashboard: false, calendar: true, gantt: true }); }
