...
function ScheduleControl({ owner, label, value, open, setOpen, onSave, onUseLatest, busy, compact = false, error }: { owner: string; label: string; value: ChecklistScheduleDto; open: boolean; setOpen: (open: boolean) => void; onSave: (value: SaveChecklistScheduleRequest) => void; onUseLatest?: (value: ChecklistScheduleDto) => void; busy: boolean; compact?: boolean; error?: ScheduleError }) {
  ...
  useEffect(() => { setDraft(scheduleDraft(value)); }, [value]);
  ...
  return <>
    ...
    {open && !readOnly && <AnchoredPopover context={floating.context} floatingStyles={floating.floatingStyles} initialFocus={0} onKeyDown={floating.onKeyDown}><div id={id} className="subtask-popover__content" role="group" aria-label={label}>
      ...
      {error?.current && <div className="subtask-schedule__conflict" role="status"><strong>Latest schedule · v{error.current.version}</strong><span>{formatSchedule(error.current)}</span><button type="button" className="button button--secondary" disabled={busy} onClick={() => { onUseLatest?.(error.current!); close(); }}>Use latest</button><span>Review and reapply your draft, then Save.</span></div>}
      <div className="subtask-popover__actions"><button type="button" className="button" disabled={busy} onClick={() => { onSave({ expectedVersion: error?.current?.version ?? value.version, schedule: scheduleInput(draft) }); close(); }}>Save</button><button type="button" className="button button--secondary" disabled={busy} onClick={() => { setDraft(scheduleDraft(value)); close(); }}>Cancel</button></div>
    </div></AnchoredPopover>}
  </>;
}
...
function SortableSubtaskRow({ item, users, busy, editing, draftTitle, popover, setPopover, scheduleError, onUpdate, onUseLatest, onRemove, onBeginEditing, onEndEditing, titleInputRef, itemRef, gripRef }: { item: Subtask; users: MentionableUser[]; busy: boolean; editing: boolean; draftTitle: string; popover: ActivePopover; setPopover: (value: ActivePopover) => void; scheduleError?: ScheduleError; onUpdate: (body: Record<string, unknown>, action: string) => void; onUseLatest: (schedule: ChecklistScheduleDto) => void; onRemove: () => void; onBeginEditing: () => void; onEndEditing: () => void; titleInputRef: (element: HTMLInputElement | null) => void; itemRef: (element: HTMLElement | null) => void; gripRef: (element: HTMLButtonElement | null) => void }) {
  ...
  return <article ref={combinedNodeRef} style={style} className={`subtask-checklist__item${item.done ? " subtask-checklist__item--done" : ""}${activeKind ? " subtask-checklist__item--popover-open" : ""}${isDragging ? " subtask-checklist__item--dragging" : ""}`} onBlur={(event) => {
    ...
    <div className="subtask-checklist__summary">
      ...
      <ScheduleControl owner={item.id} label={`Schedule for ${item.title}`} value={item.schedule} error={scheduleError} open={activeKind === "schedule"} setOpen={(open) => setKind("schedule", open)} onSave={(schedule) => onUpdate({ schedule }, "schedule")} onUseLatest={onUseLatest} busy={busy} />
...
export function SubtaskChecklist({ projectId, onAccessFailure }: { projectId: string; onAccessFailure?: (error: unknown) => void }) {
  ...
  function replace(updated: Subtask) { const next = ensureSchedule(updated); setSubtasks((current) => current.map((item) => item.id === next.id ? next : item).sort((a, b) => a.position - b.position || a.id.localeCompare(b.id))); queryClient?.setQueryData<Subtask[]>(projectDataKeys.subtasks(projectId), (current) => current?.map((item) => item.id === next.id ? next : item)); queryRuntime?.publish(createProjectDataInvalidationMessage(projectId, [{ kind: "subtasks" }])); }
