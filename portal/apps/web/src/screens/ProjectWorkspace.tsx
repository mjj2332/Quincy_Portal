...
export function ProjectWorkspace({ projectId, notice, onNoticeShown }: { projectId: string; notice?: string | null; onNoticeShown?: () => void }) {
  ...
  return <main className="work">
    ...
    <section className="workmain"><div className="wsbar"><InternalLink className="chip" to="/">← Dashboard</InternalLink><span className="ey">{activeTab === "raw" ? `RAW capture · ${rawCollection?.receivedCount ?? 0} received` : `${collectionLabel(activeTab)} collection`}</span><div className="grow" />{canUpload && activeTab === "raw" && <button className="chip" type="button" disabled={isSyncing || !(project.rawFolderPath || project.rawFolderLink)} onClick={() => void syncDropbox()}>Sync Dropbox</button>}</div>
