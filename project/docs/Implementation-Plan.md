...
- **Tonomo mapping** — `QP.fromTonomo()` + two real payload samples in `project/uploads/` make the webhook mapper directly implementable.
...
| 6 | **Tonomo**: webhook signing key exchanged; event contract (`order.created`/`updated`) confirmed; test payloads (already in `project/uploads/`) | |
