...
- [ ] **BLOCKED — R2 not enabled on this Cloudflare account.** `wrangler r2 bucket list` → error code 10042 "Please enable R2 through the Cloudflare Dashboard." This is a one-time account-level action only the user can do (accepting R2 terms in the dashboard) — no token/API bypass. Needed for MEDIA bucket `quincy-portal-media` before Phase-1 uploads work end-to-end.
- [ ] Google OAuth client secrets (GOOGLE_CLIENT_ID/SECRET, BETTER_AUTH_SECRET) into .dev.vars / wrangler secrets
- [ ] First-deploy checklist: spikes ①–③ verification (image transforms now unblocked by R2 once enabled), better-auth Google account-linking test, first `wrangler deploy` in dependency order
