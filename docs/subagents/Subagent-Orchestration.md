...
| **fast-worker** | `Agent` tool, `subagent_type: fast-worker` (`~/.claude/agents/fast-worker.md`) | Claude Opus 5.5 | **Default builder** — mechanical, well-specified work: boilerplate, tests, formatting, straightforward edits. Stops and reports back on anything ambiguous or design-level rather than guessing |
| **deep-reasoner** | `Agent` tool, `subagent_type: deep-reasoner` (`~/.claude/agents/deep-reasoner.md`) | Claude Opus 5.5 | Reasoning-heavy phases: architecture decisions, hard debugging, algorithm design, tradeoff analysis. Returns a concise, actionable conclusion |
| **design-reviewer** | `Agent` tool, `subagent_type: design-reviewer` (`~/.claude/agents/design-reviewer.md`) | Claude Opus 5.5 | Second stage of every UI browser pass ([§2a](#2a-the-browser-pass-two-stages)): judges the screenshots against the prototype as a designer, audits Luna's PASS/FAIL table. Read-only |
| **Sol** | `codex exec` — [§3](codex-cli.md) | `gpt-6-sol`, always high effort | Diff reviewer |
| **Luna** | `codex exec` — [§3](codex-cli.md) | `gpt-6-luna`, always xhigh effort | Tester — QA execution, diagnosis, authoring/fixing test code, and the mechanical stage of every Chrome/browser pass ([§2a](#2a-the-browser-pass-two-stages)) |
