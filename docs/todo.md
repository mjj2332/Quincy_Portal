...
- **TB8-03 (Project Workspace rail — Stage, Shoot, Deadline, Team, Client, Collections, Dropbox —
  and reached-collection controls: link add/remove and delivered state) is deployed to production,
  2026-09-02** (commit `77187a5` direct to `main`; app Worker `9a021a5e` only,
  background/webhook-ingress not redeployed as the diff is frontend/CSS only; no migration;
  rollback target app `7248b49a`). Migrates those surfaces off the legacy CSS classes onto Tailwind
  utilities over Quincy's design tokens, retiring the superseded `.rail` / `.project-team*` /
  `.project-deadline*` / `.collection-*` selector families per plan §7. Built through the
  Claude-only frontend pipeline (`docs/Subagent-Frontend-Orchestration.md`, generalized out of this
  plan in the same commit and cross-referenced from `AGENTS.md`/`CLAUDE.md`): Opus plan, two Sol
  scope rounds, Opus plan-approval, Sonnet builder, 11 rounds of fresh-Sol diff review, Opus
  visual/taste review in the browser. **Round 11 caught a genuine production defect that code-level
  review could not see**: a pre-existing *unlayered* legacy `.grid` rule in `app.css` silently
  outranked Tailwind's `grid`/`grid-cols-*` utilities (unlayered always beats layered under CSS
  `@layer` cascade rules), collapsing `.rail-kv` and `.project-team__member` into broken column
  splits at runtime — root-caused and fixed by renaming the legacy selector to `.legacy-grid` at
  both declaration sites and its sole consumer (`PhotoGrid.tsx`), not patched per call site. Plan
...
- **TB8-09 (the review Lightbox — the stage, the markup toolbar, the filmstrip, the compare
  layout and the review side panel) is DEPLOYED TO PRODUCTION, 2026-09-04** — merge `150e40c`, app
  Worker `7c7cdafe` only (background/webhook-ingress not redeployed; `apps/web` only), no
  migration, rollback target app `1b35ead6`. Ranking candidate #9.
  (`docs/plans/implemented/TB8-09-Lightbox-Controls-Visual-Plan.md`.) Slices `3a3f387` `90bb4e1` `0d3453a` `4d3ad17` `b16c152`
  Slices `3a3f387` `90bb4e1` `0d3453a` `4d3ad17` `b16c152` `df071ac`, plus the gate's own fixes
  `016e891`. Post-deploy check: `/api/health` 200, and the production stylesheet confirmed to carry
  the inverse scope's `--focus-ring:var(--paper-050)` and zero transparent-outline rules.
...
- **TB8-08 (the Staff Notice Board — the collapsed toggle, the post, its meta line, the owner
  ...
  `apps/web` alone), no migration, rollback target app `3be901b7`. Ranking candidate #8.
  (`docs/plans/implemented/TB8-08-Notice-Board-Visual-Plan.md`.) Slices: `0a36964` `3504e30`
  `72c2325`. `app.css` retirement is total for this surface — `grep -c "notice-board"` → **0**.
  **The headline fix is a touch-target one**: `Edit` and `Delete` were **20.5 × 24** and
  **34.2 × 24**; both axes now clear 44 at and below the `max-[721px]` breakpoint and hold 38
  above it. Three meta sites (timestamp, "edited", mention hint) moved from **3.57:1 to 9.2:1**
  with nothing regressed, the `<time>` element stopped acting as a **1,138.8px** layout strut
  (now 102.8px), and a 480px wrap defect that orphaned `Delete` onto its own row at the far left
  is gone. 1,742 tests green across six workspaces plus `packages/shared`; all 61 legacy test
  sites migrated, none deleted.
  **The plan cost two Sol rounds and 15 findings — every one verified against the repo before
  acting, and every one upheld.** Four were the draft being outright wrong rather than thin:
  `buttonClasses("text")` **cannot** reach 44×44 (it carries no `min-width` at all, and its
  `min-h-[32px]` beats BASE's `min-h-[38px]`); a bare `text-destructive` is a no-op against the
  variant's own `!text-foreground-secondary`; a bare `META_TEXT` would have uppercased the
  composer hint — **TB8-07's gate defect #3, about to ship a second time**; and a claimed
  wash/border equivalence between `Notice` and the rule it replaced that did not hold on any of
  the four values. Sol also caught the plan sitting below the implementation-detail resolution the
  frontend pipeline requires, which is what forced §5's exact per-element, per-state class
  specification. Two counting lessons: `grep -c` counts **lines, not occurrences** (48 vs the real
  49 — one line carried two), and `gap: var(--space-1) var(--space-3)` is **row-then-column**, so
  the inline gap was 12px, not the 8px the draft assumed — which withdrew a WCAG 2.5.8 failure
  claim outright, leaving the 2.5.5 / repo-contract rationale to stand on its own.
  **The gate found one defect Sol did not**, and one of its own first-pass measurements was
  wrong: a `querySelectorAll("span")` search matched the outer label-stack span rather than the
...
- **TB8-10A (the Dashboard toolbar and seven deferred defects) is DEPLOYED TO PRODUCTION, 2026-09-04**
  ...
  rollback target app `7c7cdafe`. Post-deploy verification: `/` and `/api/health` 200; the served
  stylesheet is **byte-identical** to the local build (sha256 `f4ef616c…`); `.tile:focus-visible`
  is live in the served CSS and `.tile`'s base rule no longer carries `outline: 0`, so **the WCAG
  2.4.7 photo-grid failure is fixed in production**. (commit `f320b25`; plan `docs/plans/TB8-10A-Dashboard-Toolbar-And-Deferred-Defects-Visual-Plan.md`).
...
- **TB8-06 (the Kanban board — columns, cards, card controls, the move-to popover, drag overlay and
  ...
  stylesheet byte-identical to the local build, all markers live, zero legacy board rules. Visual
  gate passed in a real browser at three viewports before merge (plan §0). (Built 2026-09-03,
...
- **TB8-05 (Notification preferences screen plus four app-shell defects) is deployed to
  ...
  redeployed (frontend/CSS only); no migration; rollback target app `c620c514`). Same Claude-only
  frontend pipeline. Clears the three shell defects TB8-04 deliberately left unfixed — B1 the
  impersonation-banner Exit invisible on hover, B2 `.topbar__user` overflow, B3 the 32px brand
  target — **and one TB8-04 did not know about**: B4, the mobile menu panel computed
  `display: none` at *every* width, so since TB8-02 (`c5e246f`) the app had no phone navigation at
  all — no Dashboard link, no Notification preferences, no Sign out. `app.css:67`'s unlayered
  `.topbar__menu-trigger, .topbar__mobile-menu { display: none }` won because TB8-02 moved the
  panel onto the shared `Menu` primitive whose `PANEL` declares no `display`. Found in Sol round 1,
  folded in per plan §12.0 rather than hotfixed ahead. **B2's band was mis-scoped by TB8-04**: it
  recorded 721–747px, but re-measurement put it at **721–1006px** — landscape tablets included — so
  the topbar now has four measured bands (≥1007 / 771–1006 / 721–770 / ≤720) rather than two.
  17 `app.css` rules retired, 1 expanded, 0 added. Drift-register rows `TB0-VIS-05` and
  `TB0-VIS-06` opened and resolved. §5 gate green — typecheck 6/6, `@quincy/web` build, **1580
  tests passing / 1 skipped / 0 failures** across all seven suites (`npm run test --workspaces`
  exits 1 on the documented `packages/shared` missing-`test`-script condition, not a regression).
  Browser acceptance walked at 1440×900 / 1024×768 / 390×844 plus all three band boundaries
  (1008/1007/1006, 772/771/770, 721/720) by Luna running unsandboxed with Chrome — the first use of
  that lane, now recorded as `Subagent-Orchestration.md` §2.11. Post-deploy passive verification on
  prod: 13/13 requests 200, zero console messages at any level, and all four defects confirmed
  fixed live (panel `display:block` with all four links, brand 44px, no horizontal overflow).
  **The visual/taste gate also caught what the mechanical pipeline missed**: the builder skipped
  §11 artefacts 8 and 8b entirely, so neither drift-register row existed until this session added
  them. Two new `docs/lessons.md` entries: the unlayered-cascade trap has a second door through
  `tokens/base.css` (a *shorthand* `:focus-visible` that resets `outline-color`, not merely
  outranks it), and a grep gate that cannot fail is not a gate — three vacuous gates in two
  releases now. Plan doc moved to
  `docs/plans/implemented/TB8-05-Notification-Preferences-And-Shell-Defects-Visual-Plan.md`.
  **One defect found at the gate and deliberately deferred**: the mobile menu has no scrim, so at
  390px the page `<h1>` is sliced mid-word at the panel's left edge — it reads as a rendering bug
  though it is not. That surface is `menu.tsx` (TB8-02's), so it goes to the new **TB8-10** deferred
  register (`docs/plans/TB8-10-Deferred-Defects-And-Cleanup-Sweep-Plan.md`, item D-01) rather than
  widening this release.
- **TB8-10 (deferred defects and the app-wide cleanup sweep) is drafted as a register, not built**
  (`docs/plans/TB8-10-Deferred-Defects-And-Cleanup-Sweep-Plan.md`, 2026-09-03). It collects the work
  earlier TB8 releases deliberately deferred, so none of it is carried only in a closed release's
  prose: D-01 the missing mobile-menu scrim (`menu.tsx` renders `MenuPrimitive.Portal` with no
  `Backdrop` — the one item that does not depend on later convergence, so it may be taken as a
  standalone fix); D-02 the duplicate `.sr-only` (`app.css:899`/`:911`); D-03 the dead
  `.app.acc-olive` block (`:719-725`, zero consumers, re-verified 2026-09-03); D-04
  `production-calendar.css:175` reaching Athelas through an **undefined** `--font-serif`'s fallback
  instead of `--type-body-serif`; D-05 `--text-muted`'s 3.6:1 across **41 surviving `app.css` paint
  sites**, of which the plan must separate text roles from decorative ones rather than sweeping all
  41; D-06 retiring `.button` (**not dead** — live consumers in `App.tsx:106` and
  `ProjectWorkspace.tsx:330,338` plus two DOM test suites); D-07 the carried base/Preflight
  decision. It also records the **verification debt** — TB8-04 §10.2 items 3/5 and TB8-05 §10.2
  items 1-17 were verified structurally, never audibly, on the TB5C waiver precedent; closing it
  needs real assistive technology and is an owner decision, not a candidate's. **It is ranking
  candidate #10, not #6** (#6 remains Board filters/card controls) and runs after candidates
  #6-#9, per the owner's instruction to visit these once the existing tracer bullets are finished.
- **TB8-04 (remaining project and Admin forms — the two project forms plus all four Admin tabs,
  and the sign-in screen) is deployed to production, 2026-09-03** (branch
  `feat/tb8-04-project-and-admin-forms`, commit `9f1a5e3`, merged to `main` `a371671`, closeout
  `6bf3505`; app Worker `c620c514` only, background/webhook-ingress not redeployed (no change); no
  migration; rollback target app `9a021a5e`, TB8-05 Notification Preferences and app-shell defects deployed to production 2026-09-03 — no migration, merge to main (`ffc3e79`), app Worker `fd65055a` only, background/webhook-ingress not redeployed (frontend/CSS only), rollback target app `c620c514`, TB8-06 Kanban board convergence deployed to production 2026-09-03 — no migration, merge to main (`b8c650d`), app Worker `18356d70` only, background/webhook-ingress not redeployed (frontend/CSS only), rollback target app `fd65055a`). Same Claude-only frontend pipeline. **The browser
  visual/taste gate found two real defects that every code review had passed**, both fixed before
  shipping: (1) the shared `FIELD_BOX` string in `components/ui/input.tsx` used a bare Tailwind
  `read-only:` variant, and because a `<select>` matches `:read-only` *unconditionally* (only
  input/textarea/contenteditable are ever `:read-write`), every `NativeSelect` in the app painted
  sunken-tan as if disabled — fixed with a `[&:read-only:not(select)]:` guard; (2) the breakpoint
  pair was written `max-[720px]`/`min-[720px]`, but Tailwind v4's `max-[Npx]` compiles to
  `width < N` (exclusive), so exactly 720px matched neither rule — the repo convention is now
  `max-[721px]`/`min-[721px]`, which are exactly complementary. Both mechanisms, plus a third
  (`display:block` on a responsive table destroying its implicit a11y roles, which must be
  explicitly reapplied in the same commit) are written up in `docs/lessons.md`. §10.2 gate: 14 of
  16 items PASS at 1440×900 / 1024×768 / 390×844; items 3 and 5 are reported honestly as
  *structurally* verified only (roles and ARIA wiring confirmed in the DOM, not audibly with a
  screen reader). §5 gate green — 1569 tests passing across the five workspaces plus 144 in
  `packages/shared`. Post-deploy passive verification on prod `/admin`: 38 requests all 200/304,
  zero app console errors, read-only select fix confirmed live. Plan doc moved to
  `docs/plans/implemented/TB8-04-Project-And-Admin-Forms-Visual-Plan.md`. **Three pre-existing
  TB8-01 app-shell defects were found during this gate and deliberately left unfixed** (confirmed
  byte-identical at HEAD, out of scope for this diff): the impersonation-banner Exit button is
  invisible on hover (Ink-on-Ink, 1:1 contrast); `.topbar__user` overflows horizontally in the
  721–747px band; `.topbar__brand` is a 32px touch target below 721px. Fold these into a later TB8
  candidate.
...
- [ ] **P2** Port TB7's uniform response-order fence back to `project-comments.ts` (required
  follow-up recorded by `docs/plans/implemented/Revamp-TB7-Notice-Board-Migration-Plan.md` §3). Its
...
- **`Revamp-TB6-Project-Card-Detail-And-Discussion-Plan.md`** (+ `docs/plans/tb6/`) — **deployed to
  production 2026-08-31**, merged to `main`, **app Worker `6f7a22b2` only** (migration `0038`
  applied — one additive index on `project_activity_events(project_id, occurred_at DESC, id DESC)`,
  no other schema change; background/webhook-ingress not redeployed — bundle-compared: webhook
  byte-identical, background's diff traced to inert shared-barrel additions), rollback target app
  `10778f00` (the prior live version). A URL-addressable "Project Quick-Detail" sheet (Overview/
  Activity/Discussion tabs) opened from the Dashboard's List/Kanban/Calendar views, reusing TB2
  freshness, TB3 discussion, TB4C activity, and TB4E external projection — no new domain store. New:
  `GET /api/projects/:id/activity` (the first read endpoint on `project_activity_events`, previously
  write-only via TB4C fan-out), the `viewQuickDetail` capability (admin/editor/external_editor,
  Photographer denied), the `invalidateProjectSurfaces` cache-coherence coordinator (`producer`
  param distinguishing the mutating surface's own settle refetch from cross-tab convergence). Built
  in 6 slices, each individually Sol-reviewed; 2 whole-branch Sol review passes (Pass B alone found
  3 Blocking + 6 Should-fix real cross-slice issues — the highest defect count of any TB6 round);
  Opus final-draft APPROVE; Agy 7-item local-dev QA PASS; post-deploy passive production check PASS
  (0 console errors, 0 failed requests). Full history: `docs/plans/implemented/
  Revamp-TB6-Project-Card-Detail-And-Discussion-Plan.md`'s own Status line.
