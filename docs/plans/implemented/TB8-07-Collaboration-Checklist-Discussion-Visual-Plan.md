...
| 7 | 872-878 | `__tabs`, `__tab`, `:hover`, `.is-active`, `:focus-visible`, `__panel`, `__panel[hidden]` | **R**. Two notes the plan owed the builder and did not give: `__panel`'s successor is just `min-w-0` (its rule's only property), and `__panel[hidden]{display:none}` needs **no** successor — the UA default already hides `[hidden]` and this app ships no `[hidden]` reset (grepped) |
| 7 | 880, 881 | the standalone `__tabs` bleed, the standalone `__panel` | **R** |
| 7 | 882-885 | the `@720` block (2 rules) | **R** → `max-[721px]:` variants |
| 7 | 862-867 | `.project-collaboration-only`, `-summary`, `-summary h2`, `__facts`/`__team`, `__team`, `-summary .member` — 6 rules | **R**; `.project-collaboration-only` itself → **H** — its rule goes, but the class name stays on both `<main>` elements as a test hook (8+ assertions in `ProjectWorkspace.dom.test.tsx` query it) with its `display:grid; gap` moved inline. Same disposition, and same reason, as `.subtask-checklist__title-trigger` |
...
| 11 | **Six `data-testid` attributes added** — `discussion-comments`, `discussion-composer`, `discussion-read-anchor` (slice 5); `project-collaboration-head`, `-scroll`, `-unread` (slice 7) | Discussion, Panel | Added during the slice-5 and slice-7 builds. Five retired `.project-collaboration__*` classes turned out to be **query selectors in four test files**, and the alternatives were worse: loosening the assertions (forbidden) or inventing accessible names that do not exist on a list wrapper, a form, and an `aria-hidden` scroll sentinel. Slice 7's three are the same story on structural assertions — DOM order via `firstElementChild`/`children[2]`, and `document.activeElement` identity — that no accessible name can express. `data-testid` is inert, carries no semantics, displaces no `aria-*`, and is an **established pattern in this repo** — `ConfirmDialog.tsx`, `ProductionCalendarFoldChoice.tsx`, `ProductionCalendarScheduleEditor.tsx` and others already use it. Listed here rather than waved through as "a class change" |
