...
5. Statement 1's normalized `UNION ALL` branches emit project Deadline events, checklist events, and unscheduled Project/checklist candidates. Window functions assign `row_number()` and `count(*) over()` independently to each Unscheduled kind; only ranks `<= 50` are returned while total counts survive for truncation copy. Scheduled branches also carry `count(*) over()` across the combined scheduled-event stream. If the count exceeds `PRODUCTION_CALENDAR_MAX_SCHEDULED_EVENTS = 10_000`, the SQL emits only a density sentinel and the handler returns non-200 `422 calendar_range_too_dense` with `{count,max}` and refinement copy; it never silently truncates scheduled events.
...
The dependency slice must capture matched screenshots of all five official shadcn flavors at 1440x900, 1024x768, and 390x844 against the current Dashboard evidence. Render them through a throwaway Vite dev-only harness/story route that imports the candidate component and fixture; delete the harness route and its entry before the Slice 3 commit, and prove it is absent from the production build graph. Thus screenshots are possible while no product route or shipped bundle renders Calendar yet. Pulse is accepted only if its spacing/focus/geometry needs the fewest structural overrides. If evidence selects another flavor, record the comparison and pin that one before Slice 5; this is a one-time implementation evidence choice, not runtime theming.
...
- Range drag shifts both endpoint dates by the same calendar-day delta. Each endpoint keeps its own wall-clock `HH:mm`; each is independently re-resolved through `resolveSydneyCivilMinute`. Do not preserve elapsed milliseconds across DST.
...
- Add smoke tests that import the retained component/plugins/styles and render FullCalendar date output (its internal Temporal path) under Vite 8 build, happy-dom, and Node configs. Prove `packages/shared`, `workers/app`, and the Quincy adapter/mappers import neither `Temporal` nor `temporal-polyfill`.
- Before any product UI slice, run a real browser whose operating/browser zone is not Sydney against a fixture spanning the April 2026 fold and October 2026 gap. Render Month, 24-hour Week, and List/Agenda with `timeZone="Australia/Sydney"`; assert timed ISO instants display at the correct AEST/AEDT offsets, all-day `YYYY-MM-DD` values retain their dates/exclusive ends, all 24 Week hour labels are Sydney-correct, and `eventDrop`/`eventResize`/external `drop`/`dateClick` round-trip through `fullCalendarCallbackToSydneyCivil` without reading the browser zone. Record the browser zone and callback payloads as evidence.
...
Acceptance: exact dependency/lock/CSS delta resolves, typechecks, builds, and passes import/polyfill/style smoke tests with no peer or provenance failure; the non-Sydney real-browser gap/fold rendering and callback proof passes; and the deleted flavor harness is absent from the shipped bundle. Because no product route references the component yet, this slice makes no production chunk/network claim; routed chunk/network verification belongs to Slice 5.
