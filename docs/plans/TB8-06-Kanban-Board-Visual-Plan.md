...
| 11 | `.kcol__head .ey` | **R** — `[&_.ey]:!leading-[1.25]` **on `.kcol__head` itself** (§3.3). *Corrected during slice 2:* the plan first wrote a bare `!leading-[1.25]`, which has nowhere to go — `.ey` under `.kcol__head` is rendered inside `StatusBadge`, which takes only `{ stageKey }`. That is the same fact row 13 cites to stay **K**, so the two rows contradicted each other. The arbitrary-descendant variant reproduces the original selector without touching `atoms.tsx`; verified in the built CSS as `.[&_.ey]:!leading-[1.25] .ey{…}`, scoped, not global. **First use of the `[&_…]:` variant in this repo** — later slices needing a descendant selector should follow it rather than reach into a shared component |
