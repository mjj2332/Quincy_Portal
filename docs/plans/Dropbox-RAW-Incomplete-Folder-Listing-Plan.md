...
## Diagnostic progress made without live Dropbox access (prod D1 + source, 2026-07-29)

Two more checks that don't need a Dropbox API call, run to close out Open Question 3 as far as
D1 alone allows:

- **There is exactly one Dropbox connection** (`integration_connections`, `id: ea29a4dd-…`),
  `status = 'connected'`, `last_error IS NULL`, created 2026-07-20 07:59:29 — matching
  `docs/todo.md`'s "sharing.read scope added + connection re-authorized" entry for that date.
  `getConnection()`'s "resolves to one canonical row" behavior (Branch E's premise) is therefore
  moot in practice, not just in theory: there is only one row to resolve. This closes the
  "multiple connections, wrong one picked" reading of Open Question 3 outright.
- **Its granted OAuth scope string does not include `sharing.read`**, despite the todo.md entry.
  Confirmed against the actual write path, not assumed: `integrations.ts`'s OAuth callback sets
  `scopes` directly from Dropbox's token response — `scopes: typeof token.scope === "string" ?
  token.scope : null` — so this is what Dropbox actually granted at last connect, not stale
  bookkeeping. Granted: `account_info.read files.content.read files.content.write
  files.metadata.read`. **This does not implicate Branch B or E for the current symptom** —
  `sharing.read` is only exercised by `getSharedLinkMetadata()`
  (`pathFromRawFolderLink()` → `dropbox/client.ts:550`'s error message names it explicitly), which
  `sync.ts:187` only calls when `raw_folder_path` is empty; this project has `raw_folder_path` set,
  so that path is never reached. It's a genuine, separately-worth-fixing gap (`raw_folder_link` is
  set on this project too but is dead code as configured, per Fix branch A step 1) — flag it to the
  studio, but it is not this bug's mechanism.
- **`dropbox_monitor_health`'s `raw` scope row is current and clean**: `root = '/Tonomo/Raw Files'`
  (matches every delta job's payload), `last_error IS NULL`, `last_successful_page_at` = 2026-07-29
  01:17:19 (about two hours before this check), `reset_count = 0`. The delta monitor is healthy
  right now — consistent with the cross-project evidence that it ingests fine for eight other
  projects, and adds nothing new for 17 Oxford Street specifically (this table is keyed by
  connection+scope, not per-project, so it can't localize to one project's folder).

**What's still blocked, and why:** the plan's "Fast path" (a human opening the Dropbox folder) and
the full six-step protocol both require either a Dropbox-authenticated human or the connection's
decrypted OAuth token (encrypted in D1 under `INTEGRATION_KEK`, a Worker secret this session has
no access to) to make a live `files/list_folder` call. Neither is available in this session.
**Next action is unchanged: get a human to check for `Listing Images/` at the configured path** —
everything queryable from D1/source without that has now been extracted.
...
3. ~~Which Dropbox account does the studio's resolved connection actually authenticate as~~ —
   **partly answered from D1 (see "Diagnostic progress" above): there is exactly one connection,
   ruling out a wrong-connection-picked reading.** Still open: which *human* Dropbox
   account/email that one connection is, and whether it's the same one that uploaded the 255 —
   that still needs the live `users/get_current_account` call in diagnostic step 1, or simply
   asking whoever holds the studio's Dropbox login.
