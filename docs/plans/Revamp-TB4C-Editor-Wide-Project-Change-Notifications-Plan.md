...
**Status:** APPROVED FOR BUILD (2026-08-27). Pipeline: Sol draft → Sol review r1 (4 Blocking / 4 Should-fix) → Sol revision pass 1 (all 8 resolved) → Sol review r2 (all confirmed, +1 Should-fix / 1 Nit) → Opus plan-tier review (revert round 1 of 2: all 8 fixes confirmed sound vs landed code, plan structure approved, 7 surgical revert items) → Sol revision pass 2 (all 7 resolved) → Opus revert round 2 (final): **APPROVE**, all 7 verified against source, item 3 (`deliverBroadInApp()` contract) confirmed airtight, 7 trivial nits N1–N7 to fold in during the build. Not yet built.
...
This document is the initial Sol-authored draft with revision pass 1 applied.

1. Round-1 fresh-Sol findings are resolved in this revision; a fresh Sol performs review round 2.
2. After fresh-Sol approval, a fresh Opus plan-tier session reviews it, with no more than two reverts to Sol.
...
| `project.checklist.item_deleted` | `checklist` | `project-checklist:<itemId>:deleted`; `{ itemId, checklistTitle }` | DELETE in `portal/workers/app/src/routes/project-subtasks.ts:166-173` | One per winning deletion, carrying the bounded pre-delete title. |
...
Add a small channel-plan dispatch to `processNotificationMessage()` so `project.activity.broad` completes after terminal in-app handling. All other event types continue through their existing in-app/email behavior unchanged.
